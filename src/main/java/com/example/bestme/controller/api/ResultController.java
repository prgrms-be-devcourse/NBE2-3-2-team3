...
public class ResultController {
    ...
    public ResponseEntity<ApiResponse<ResultResponse.CreateResultResponseDTO>> createResult(HttpServletRequest request, @RequestBody ResultRequest.CreateResultDTO createResultDTO) {
        ...
        String requestMessage = "퍼스널 컬러 진단을 위한 얼굴의 여러 색상 코드를 알려드리겠습니다. 피부 톤에서는 가장 밝은 부분이 #" + createResultDTO.getLightestSkinColor() + ", 가장 어두운 부분이 #" + createResultDTO.getDarkestSkinColor() + "입니다. " +
                "입술 본연의 색상은 #" + createResultDTO.getLipColor() + "입니다. 자연 머리카락 색상은 #" + createResultDTO.getHairColor() + "이며, " +
                "눈동자의 기본 색상은 #" + createResultDTO.getPupilColor() + ", 홍채 외곽은 #" + createResultDTO.getIrisColor() + "입니다. " +
                "종합적으로 봤을 때 가장 가까운 퍼스널 컬러는 무엇인까요?" +
        ...
        return ResponseEntity.ok(ApiResponse.success(gptRequest.substring(4), dto));
    }
