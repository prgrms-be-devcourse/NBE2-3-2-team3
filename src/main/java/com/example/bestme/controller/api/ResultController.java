...
public class ResultController {
    ...
    public ResponseEntity<ApiResponse<ResultResponse.CreateResultResponseDTO>> createResult(@PathVariable Long userId, @PathVariable Long colorId, @RequestBody ResultRequest.CreateResultDTO createResultDTO) {
        ...
        String requestMessage = "제 피부 톤은 가장 밝은 부분이 " + createResultDTO.getLightestSkinColor() + ", 가장 어두운 부분이 " + createResultDTO.getDarkestSkinColor() + "입니다. " +
                ...
                "주어진 답변으로 응답이 어렵다면 5를 주세요.";
        System.out.println(chatGPTController.chat(requestMessage));
        Long colorCode = null;
        if (chatGPTController.chat(requestMessage).equals("1")) { colorCode = 1L;} else if (chatGPTController.chat(requestMessage).equals("2")) {colorCode = 2L;
        } else if (chatGPTController.chat(requestMessage).equals("3")) { colorCode = 3L; } else if (chatGPTController.chat(requestMessage).equals("4")) { colorCode = 4L;
        } else { colorCode = 5L; }
        Result result = resultService.createResult(userId, colorCode, createResultDTO);
